...
Mat rotateTemplate(const Mat& src, double angleDegrees) {
	...
	Mat rotated;
	warpAffine(src, rotated, rotationMatrix, src.size(), INTER_LINEAR, BORDER_CONSTANT, Scalar(0));
	return rotated;
}
...
int GeoMatch::CreateGeoMatchModel(Mat templateArr, double maxContrast, double minContrast)
{
	...
	*/
	...
	for (int angle = 0; angle < 360; angle += 15) {
		Mat rotatedTemplate = rotateTemplate(templateArr, angle);
